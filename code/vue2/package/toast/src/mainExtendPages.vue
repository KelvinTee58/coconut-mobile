<template>
  <div class="coo-toast" :class="[positionClassName, customClass, { 'is-rolling': getType == 'loading' }]" :style="[customStyle]">
    ...
    > -->
      <div class="coo-toast__content">
        <div class="coo-toast__content--icon" v-if="this.toastContent.iconName">
          <coo-icon :name="this.toastContent.iconName" :size="40"></coo-icon>
        </div>
  ...
  methods: {
    ...
    show() {
      ...
      if (this.toastContent.duration == "infinite") {
        return;
      } else if (this.toastContent.duration && !isNaN(Number(this.toastContent.duration)) && typeof Number(this.toastContent.duration) === "number")
        setTimeout(() => {
          this.hide();
